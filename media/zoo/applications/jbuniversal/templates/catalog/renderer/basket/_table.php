        ...
        <tr class="jbcart-row jsCartTableRow js<?php echo $itemKey; ?>" data-key="<?php echo $itemKey; ?>">
            ...
            <td class="jbcart-price"><?php
                if ($config->get('tmpl_price4one', 1)) {
                    // Get JBPrice instance to access discount calculation
                    $elementId = $cart->get($itemKey . '.element_id');
                    $itemId = $cart->get($itemKey . '.item_id');
                    $jbPrice = $this->app->jbentity->getItemType($itemId, $elementId);
                    
                    if ($jbPrice && method_exists($jbPrice, 'getCartPrices')) {
                        $cartPrices = $jbPrice->getCartPrices();
                        $originalPrice = $cartPrices['original'];
                        $discountedPrice = $cartPrices['discounted'];
                        
                        // Show original price crossed out if different from discounted
                        if ($originalPrice->compare($discountedPrice, '>')) {
                            echo '<span style="text-decoration: line-through; opacity: 0.7;">' . $originalPrice->html() . '</span> ';
                        }
                        
                        // Show discounted price
                        echo $discountedPrice->html();
                    } else {
    ...
    <tr class="jbcart-row-remove">
        <td colspan="6">
            <span class="jsDeleteAll item-delete-all jbbutton orange">
                <?php echo Text::_('JBZOO_CART_REMOVE_ALL'); ?></span>
        </td>
    </tr>
