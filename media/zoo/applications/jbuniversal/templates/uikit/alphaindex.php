...
$title = Text::_('JBZOO_STARTING_WITH') . ' "' . $this->app->string->strtoupper($this->alpha_char) . '"';
...
if (!$this->app->jbcache->start($this->alpha_char)) {
    ...
    if ($this->params->get('template.subcategories_show', 1) && count($this->selected_categories)) {
        echo '<h2 class="subtitle">' . Text::_('JBZOO_FOUND_AMONG_CATEGORIES') . '</h2>';
    ...
    if ($this->params->get('config.items_show', 1) && count($this->items)) {
        echo '<h2 class="subtitle">' . Text::_('JBZOO_FOUND_AMONG_ITEMS') . '</h2>';
